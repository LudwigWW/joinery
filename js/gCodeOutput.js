...
function addGCodePartsCGlobal(inString, params, placeList, commandObjs, depthAdjustment, print_Offset_X, reverse=false, durations) {
	...
	for (let place of placeListLocal) {
		...
		if (commandObj.directional == false) {
			if (pattern[patternIndex] !== 0) {
				...
				outString += getCodeWRetraction(produceCode(needleObj, place, depthAdjustment), chosenPrinter, durations.durationEstimate, durations.durationEstimateTotal, durations.toComeEstimate);
				outString += getCodeWRetraction(produceCode(commandObj, place, depthAdjustment, false), chosenPrinter, durations.durationEstimate, durations.durationEstimateTotal, durations.toComeEstimate);
			}
		}
		else {
			if (Math.abs(pattern[patternIndex]) == 0) {
					...
					if (((placeCounter+i-patternIndex+1) > placeList.length)) { // only parts of the pattern that would come after line ends 
						if ((Math.abs(pattern[patternPlace]) > 0.5)) { // Only if the pattern actually connects to something
							if (mod((i - pattern[patternPlace]), pattern.length) == patternIndex)  { // Check if it connects to the current point
								...
								outString += getCodeWRetraction(produceCode(commandObj, place, depthAdjustment, true), chosenPrinter, durations.durationEstimate, durations.durationEstimateTotal, durations.toComeEstimate);
								break;
							}
						}
					}
				...
				// "Default" outcome (for rotation variant patterns)
					...
					if (lastPlace[lastPlace.length-pattern[patternIndex]].handled == false) {
						outString += getCodeWRetraction(produceCode(needleObj, lastPlace[lastPlace.length-pattern[patternIndex]], depthAdjustment), chosenPrinter, durations.durationEstimate, durations.durationEstimateTotal, durations.toComeEstimate);
						lastPlace[lastPlace.length-pattern[patternIndex]].handled = true;
					}
					outString += getCodeWRetraction(produceCode(commandObj, place, depthAdjustment, false, true), chosenPrinter, durations.durationEstimate, durations.durationEstimateTotal, durations.toComeEstimate); // only aim
	...
	if (commandObj.postCode && lastPlace.length > 0) { // TODO: This assumes the last place is actually used, which is only true for patterns that use all holes
		outString += getCodeWRetraction(produceCode(commandObj, lastPlace[lastPlace.length-1], depthAdjustment, false, false, true), chosenPrinter, durations.durationEstimate, durations.durationEstimateTotal, durations.toComeEstimate);
	}
